...
For this post Jest was used in version _23.6.0_ together with Puppeteer _1.11.0_.
...
beforeAll(async () => {
  ...
  await new Promise((resolve) => {
    serverProcess = spawn('live-server', [
      `--port=${testUrl.split(':')[2]}`,
      '--no-browser',
    ])
...
The above verifies number _0_ from the three things we want to check during our test. Next up is the counter itself and the displayed message.
